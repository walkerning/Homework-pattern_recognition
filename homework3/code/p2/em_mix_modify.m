...
weighted_covs = zeros(d, d, length(param));
for i=1:length(param),
  ...
  weighted_covs(:, :, i) = (repmat(post_prob(:,i),1,d).*Z)'*Z;
  %param(i).cov = (weighted_cov + prior_n*param(i).prior_cov)/(post_n+prior_n);

end;
cov = (sum(weighted_covs, 3) + param(1).prior_n * param(1).prior_cov) /...
    (sum(sum(post_prob)) + param(1).prior_n);
for i=1:length(param)
    param(i).cov = cov;
end
% --------------------------------------------------------
